...
resource "aws_s3_bucket" "prod_codepipeline_bucket" {
  bucket        = "prod-${lower(var.codecommit_repo_name)}--2024"
  force_destroy = true
}

resource "aws_s3_bucket_public_access_block" "prod_codepipeline_bucket_pab" {
  bucket = aws_s3_bucket.prod_codepipeline_bucket.id
...
resource "aws_iam_role" "prod_codepipeline_role" {
  name               = "codepipeline_prod_${var.codecommit_repo_name}"
  assume_role_policy = data.aws_iam_policy_document.prod_assume_role.json
}

# resource "aws_iam_policy" "prod_codepipeline_policy" {
...
data "aws_iam_policy" "prod_codepipeline_policy" {
  name = "codepipeline_standard_policy"
}

resource "aws_iam_role_policy_attachment" "codebuild_prod_policy_attach" {
  role       = aws_iam_role.prod_codepipeline_role.name
  policy_arn = data.aws_iam_policy.prod_codepipeline_policy.arn
  # policy_arn = aws_iam_policy.prod_codepipeline_policy.arn
}

# ----- CodePipeline Resource -----

resource "aws_codepipeline" "codepipeline_prod" {
  name          = "prod_${var.codecommit_repo_name}_pipeline"
  role_arn      = aws_iam_role.prod_codepipeline_role.arn
  pipeline_type = "V2"

  artifact_store {
    location = aws_s3_bucket.prod_codepipeline_bucket.bucket
    type     = "S3"
  }
